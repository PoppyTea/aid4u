...
- **PR review follow-up (2026-08-03, revised 2026-08-16):** after opening a PR, wait 5 minutes, then check for review comments (`gh pr view <N> --json comments`). If none from CodeRabbit yet, wait another 5 minutes and check once more. If still nothing after ~10 minutes total, stop polling and tell the user directly to ping when review comments show up — don't keep silently retrying past that point. **CodeRabbit-specific (skorygowane 2026-08-25):** CodeRabbit **recenzuje automatycznie** przy zakładaniu PR-a — plan to dziś Pro Plus z limitem **1 recenzji na godzinę**, nie free tier. Wcześniejszy zapis („repo poniżej 10 gwiazdek, więc nie ma auto-review") był nieaktualny i realnie kosztował: na PR #85 recenzja pojawiła się o 21:16 przy otwarciu PR-a, ręczne `@coderabbitai review` cztery godziny później było no-opem („does not re-review already reviewed commits"), a agent czekał na wynik, który już leżał w wątku. **Gdzie szukać wyniku:** komentarz-walkthrough w `gh api repos/<owner>/<repo>/issues/<N>/comments`, NIE w `reviews` ani w `pulls/<N>/comments` — przy zerze uwag oba te kanały są puste, a walkthrough mówi „No actionable comments were generated in the recent review". Nadal nie wywołuj `@coderabbitai review` bez jawnej zgody użytkownika na konkretny PR: limit godzinowy jest realny i łatwo go spalić.
