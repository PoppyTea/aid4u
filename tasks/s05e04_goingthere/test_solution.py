...
@pytest.fixture
def going_task(monkeypatch: pytest.MonkeyPatch) -> solution.GoingThereTask:
    """Create an offline task with the kill-switch read and network client isolated."""
    monkeypatch.setattr(solution, "check_abort", lambda: None)
    task = solution.GoingThereTask(hub=MagicMock(), llm=None)
...
class TestPlanowanieMisji:
    ...
    def test_pobiera_wskazowke_i_omija_skale_w_nastepnej_kolumnie(
        self, going_task: solution.GoingThereTask
    ) -> None:
    ...
    def test_jedno_wyjscie_pomija_zbedna_wskazowke(
        self, going_task: solution.GoingThereTask
    ) -> None:
    ...
    def test_ostatni_ruch_musi_trafic_dokladnie_w_wiersz_bazy(
        self, going_task: solution.GoingThereTask
    ) -> None:
    ...
    def test_nieosiagalny_wiersz_bazy_przerywa_misje(
        self, going_task: solution.GoingThereTask
    ) -> None:
...
class TestWskazowkiApi:
    ...
    def test_druga_czytelna_wskazowka_konczy_retry(
        self, going_task: solution.GoingThereTask
    ) -> None:
    ...
    def test_wyczerpanie_retry_zawiera_wszystkie_odebrane_tresci(
        self, going_task: solution.GoingThereTask
    ) -> None:
...
class TestRozbrajanieRadaru:
    ...
    def test_czysty_skan_nie_wysyla_rozbrojenia(
        self, going_task: solution.GoingThereTask
    ) -> None:
        ...
        going_task._insist = MagicMock(side_effect=lambda request: request())
    ...
    def test_namierzenie_wysyla_frequency_i_hash(
        self, going_task: solution.GoingThereTask
    ) -> None:
...
class TestPonawianieApi:
    ...
    def test_html_jest_odrzucany_a_kolejna_odpowiedz_akceptowana(
        self, going_task: solution.GoingThereTask, monkeypatch: pytest.MonkeyPatch
    ) -> None:
    ...
    def test_wyczerpanie_limitu_zglasza_ostatni_blad(
        ...
        monkeypatch.setattr(solution.time, "sleep", MagicMock())
        ...
        assert request.call_count == 2
        assert solution.time.sleep.call_args_list == [
...
class TestWykonanieRuchu:
    ...
    def test_poprawny_ruch_zwraca_nowy_stan(
        self, going_task: solution.GoingThereTask
    ) -> None:
    ...
    def test_odrzucony_ruch_zawiera_stan_i_odpowiedz_huba(
        self, going_task: solution.GoingThereTask
    ) -> None:
    ...
    def test_odpowiedz_bez_gracza_jest_odrzucana(
        self, going_task: solution.GoingThereTask
    ) -> None:
