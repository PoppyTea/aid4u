...
- `solution.py`: `@task("s05e04", hub_name="goingthere")` — pętla gry, ponawianie zapytań
  do zagłuszanego API.
- `test_solution.py`: 48 testów offline — 33 własne (w tym **14 dosłownych komunikatów
  z żywego API** sparowanych z prawdziwą pozycją skały) plus 15 wygenerowanych przez
  CodeRabbita na warstwę orkiestracji, poprawionych ręcznie (patrz `AGENTS.md` w roocie,
  sekcja o CodeRabbicie).
