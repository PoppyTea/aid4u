...
def test_resolve_coordinates(monkeypatch):
    ...
    """
    plant = PowerPlant(
        location_name="Grudziądz", location=None, uuid="PWR7264PL", power_level=1138, active=True
    )
...
point_x = GeoPoint(name="Odbicie_Radomia_na_Zachod", latitude=51.4025, longitude=20.8773)


@pytest.mark.parametrize(
    "single_point, many_points, solution, run",
    [
        ...
        pytest.param(
            WARSAW,
            [double_of_radom, SZCZECIN, KRAKOW, RADOM, fake_radom, point_x],
            [RADOM, point_x],
            3,
            id="many_diff_nearest",
        ),
        # 4. prawdopodobne dane z atrybutów odpowiednich klas
        pytest.param(
            power_plants[4].location,
            test_dude.location_history,
            WARSAW,
            4,
            id="simulation_2_one_of_many",
        ),
        # 5 Dedup candidates
        pytest.param(
            WARSAW,
            [RADOM, SZCZECIN, KRAKOW, RADOM, KRAKOW],
            RADOM,
            5,
            id="many_nearest_with_doubles",
        ),
    ],
)
    ...
    if isinstance(expected, list):
        ...
        for i in range(len(expected)):
            for j in range(len(solution)):
                ...
                same_distance = single_point.distance_to(expected[i]) == single_point.distance_to(
                    expected[j]
                )
                assert same_distance
                distance_as_expected = single_point.distance_to(
                    expected[i]
                ) == single_point.distance_to(solution[j])
        ...
        good_predction: bool = (expected.latitude == solution.latitude) and (
            expected.longitude == solution.longitude
        )
...
@pytest.mark.skip(
    reason="TDD cycle 2 - test jeszcze niezaprojektowany: brak modelu Answer (payload do /verify)"
)
...
def test_suspect_location_history_coords_list():
    test_dude: Suspect = Suspect(
        name="John", surname="Kowalski", born=1992, location_history=[KRAKOW, WARSAW]
    )
...
def test_suspect_location_history_path_traversal_prevention():
    with pytest.raises(ValueError, match="Path traversal detected"):
        Suspect(name="John", surname="Nowak", born=1972, location_history=Path("/etc/passwd"))
...
def test_nearest_suspect_returns_distance_and_identity():
    plant = PowerPlant(
        location_name="Radom", location=RADOM, code="PWR8406PL", power_level=38, active=True
    )
...
def test_nearest_suspect_returns_none_when_location_unresolved():
    plant = PowerPlant(
        location_name="Radom", location=None, code="PWR8406PL", power_level=38, active=True
    )
...
def test_search_suspect_history_for_nearest_power_plant_returns_none_code_when_no_plant_resolved():
    unresolved_plants = [
        PowerPlant(
            location_name="Nieznane", location=None, code="PWR0000PL", power_level=1, active=True
        )
    ]
