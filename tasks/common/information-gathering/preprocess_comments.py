...
def process_comments(filepath, allowed_dir=None):
    if allowed_dir is None:
        allowed_dir = os.path.abspath(os.getcwd())
    else:
        allowed_dir = os.path.abspath(allowed_dir)

    allowed_dir_with_sep = allowed_dir if allowed_dir.endswith(os.sep) else allowed_dir + os.sep
    abs_filepath = os.path.abspath(filepath)

    if abs_filepath != allowed_dir and not abs_filepath.startswith(allowed_dir_with_sep):
        raise ValueError(
            f"Path traversal detected: {filepath} is outside of allowed directory {allowed_dir}"
        )
